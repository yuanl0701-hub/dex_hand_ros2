...
2026-08-04，当前实体手装配人员确认了本机映射：ID 1--4 依次为食指、中指、无名指、
小指弯曲，ID 5 为拇指弯曲，ID 6 为拇指对掌/旋转。该映射是当前装配确认，不是原论文
证据；换手或重新分配 ID 时必须重新确认。ID 6 曾因波特率被设为 9600 而无法加入
115200 总线；装配人员将它恢复为 ID 6 / 115200 后，报告六个地址均可通信。ID 6 的
完整对掌行程仍需单独标定，当前实体手势将它保持在逻辑中位 50。
...
逻辑轴、标签、关节映射和手势属于
`config/hand_models/mpd20_six_axis/`。实体目录提供 `open`、`half_open`、`fist`、
`vgesture`、`rock` 和 `point`。按当前 `[120,850]` 标定换算，四指命令端点使用 raw
约 130/840；握拳时拇指弯曲使用 raw 约 400；尚未完成对掌行程标定的 ID 6 固定在
逻辑中位 50。任意轴通信失败时均由单轴容错临时跳过。通用仿真手势不能未经机械映射
和空载验证直接用于实物。

## 9. 启动和首轮无负载验收

当前 Ubuntu/Jazzy 工作站的一键启动方式：

```bash
./scripts/start_mpd20_jazzy.sh --build --enable-motion
```

第一次构建后可省略 `--build`。该便捷脚本由节点自身执行启动检查，并按实体运行策略
隔离无响应轴；需要正式验收时仍应先单独运行只读六轴预检。Jazzy 脚本默认动作禁用，
必须显式给出 `--enable-motion`。
...
实体运行策略默认使用 `gesture_execution_mode: direct`。手势命令会按轴各写一次最终
`mpd20_max_speeds` 完成运动。此模式下 `GestureCmd.speed` 只需为正数，实际速度由实体
部署 YAML 的逐轴限速决定。Modbus RTU 仍是逐轴请求/应答，因此“直接”表示取消软件
分步轨迹，不代表六轴在完全相同的物理时刻收到指令。

这种降级模式不能保证完整手势或抓取几何。若应用要求六轴协调，应在部署覆盖文件中设
`hardware_allow_partial_operation: false`，恢复任意单轴失败即中止命令的严格模式。
无论采用哪种模式，急停和节点退出的主动保持仍是严格路径：任何轴无法确认保持都会报告
`fault`，不能视为可靠停止。实体 `direct` 手势是离散位置命令，物理运行覆盖默认设置
`command_watchdog_enabled=false`，避免空闲时因瞬时通信掉线进入严格主动保持并触发整机
故障；仿真和连续命令模式仍默认启用看门狗。硬件断电急停不受此设置影响。
...
- [ ] 六个实体手势逐一空载通过，且任意单轴临时掉线时其余轴行为符合预期；
- [ ] ID 6 对掌轴的安全 raw 行程、方向和最大速度单独完成标定；
