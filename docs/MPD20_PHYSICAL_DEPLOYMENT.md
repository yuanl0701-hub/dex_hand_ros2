...
证据；换手或重新分配 ID 时必须重新确认。目前 ID 6 暂时不可用，运行时由单轴容错跳过。
...
约 130/840；握拳时拇指弯曲使用 raw 约 400；未标定的 ID 6 固定在逻辑中位 50，并在
离线时跳过。通用仿真手势不能未经机械映射和空载验证直接用于实物。
...
第一次构建后可省略 `--build`。脚本不执行严格六轴预检，因为当前 ID 6 离线；节点自身
仍执行启动检查并将无响应轴隔离。Jazzy 脚本默认动作禁用，必须显式给出
`--enable-motion`。
...
`command_watchdog_enabled=false`，避免空闲时因已知离线的 ID 6 触发整机故障；仿真和
连续命令模式仍默认启用看门狗。硬件断电急停不受此设置影响。
...
- [ ] 六个实体手势逐一空载通过，且 ID 6 离线时其余轴行为符合预期；
