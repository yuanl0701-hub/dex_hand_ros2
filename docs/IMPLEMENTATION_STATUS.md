...
| MPD20 backend | Implemented and manual-verified; hardware pending | Vendor V1.5/Modbus Poll cross-check, fake-transport mapping/probe/motion-gate tests | `backends/mpd20.py`, `test_mpd20_driver.py` | Physical ID map, travel, direction and timing are unknown | Execute staged physical acceptance plan |
...
| State feedback | Implemented and container-verified | JointState ~99.98 Hz, TF query and rosbag; MPD20 function-04 fake transport | `hand_node.py`, `core/joint_mapping.py`, `backends/mpd20.py` | MPD20 bus polling timing unmeasured | Repeat on target host for timing |
...
| Automated tests | Implemented and verified | 68 pure tests pass locally; 69 Humble package tests pass in container | `test/` | Hardware tests blocked | Run physical acceptance suite |
