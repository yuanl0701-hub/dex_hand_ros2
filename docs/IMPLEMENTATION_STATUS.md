...
| MPD20 backend | Implemented and partially hardware-exercised | Vendor cross-check; fake-transport tests; operator reported IDs 1--5 gesture motion and ID 6 offline behavior | `backends/mpd20.py`, `test_mpd20_driver.py` | Full travel, ID 6 and timing remain unverified; partial operation reduces coordination guarantees | Execute staged acceptance and dropout tests |
...
| Command handling | Implemented and container-smoke-verified | ROS callbacks delegate to tested core; direct single-setpoint and smooth modes unit-tested | `hand_node.py`, `core/controller.py` | Physical direct-command timing unverified | Deployment-host and hardware tests |
...
| Gesture library | Implemented and unit-verified | Nine-entry generic catalogue plus six safe-margin MPD20 poses; raw-equivalent endpoint assertions | `core/gestures.py`, `config/hand_models/*/*.json` | New V/rock/point poses and ID 6 are not hardware-verified | Test each pose unloaded before use |
...
| Smooth/direct transitions | Implemented and verified | Mock direct single-command, smooth endpoint/cancellation and partial-start-feedback tests | `controller.py`, `test_controller.py` | Timing not real-time; partial poses omit unavailable axes | Measure on target |
...
| Automated tests | Implemented and verified | 80 pure tests pass locally; historical 69-test Humble package run passed before the latest physical-runtime changes | `test/` | Latest ROS build and full hardware tests pending | Rebuild on target and run physical acceptance suite |
