...
from .driver import DriverError, DriverNotConnectedError, DriverValidationError
...
class ModbusRTUProtocol:
    ...
    def read_holding_registers(self, address: int, register: int, count: int = 1) -> list[int]:
        ...
        request = self.frame(address, 0x03, struct.pack(">HH", register, count))
        response = self._exchange(request, 5 + 2 * count)
        self._validate_header(response, address, 0x03)
